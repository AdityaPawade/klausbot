...
- Re-export everything from schema and loader
  ...
  <verify>Create test file that imports config; verify it throws if BOT_TOKEN missing; verify it loads if .env valid</verify>
...
2. Create .env with BOT_TOKEN=test123, run quick test:
   ```
   node -e "
   import { config } from './dist/config/index.js';
   import { logger } from './dist/utils/index.js';
   console.log('Config loaded:', !!config.BOT_TOKEN);
   logger.info({ token: config.BOT_TOKEN.slice(0,4) + '...' }, 'Test log');
   "
   ```
...
- Config validates BOT_TOKEN (required), LOG_LEVEL (optional), DATA_DIR (optional)
