...
| 2 | Claude can call mcp__klausbot__create_cron with typed parameters | ✓ VERIFIED | tools/cron.ts:16-63 defines create_cron tool with zod schema |
| 3 | Claude can call mcp__klausbot__list_crons and mcp__klausbot__delete_cron | ✓ VERIFIED | tools/cron.ts:65-129 defines list_crons and delete_cron tools |
...
1. **Test: Create cron via MCP tool**
   ...
   - Expected: Claude calls mcp__klausbot__create_cron (not "klausbot cron add")
...
2. **Test: List and delete crons**
   ...
   - Expected: Claude calls mcp__klausbot__list_crons
   - Message: "Delete the email reminder"
   - Expected: Claude calls mcp__klausbot__delete_cron
