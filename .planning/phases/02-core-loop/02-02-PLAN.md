...
must_haves:
  truths:
    - "Claude reads identity files from ~/.klausbot/identity/ at session start"
    - "Claude retrieves past conversations when user asks about history"
    - "Claude's response style reflects identity configuration (name, personality)"
    - "Retrieval instructions tell Claude how to search conversations"
  artifacts:
...
notes:
  - "Cache invalidation: Identity files cached at first load, changes require process restart. Acceptable for Phase 2; hot-reload can be added later if needed."
---
...
- Include:
  ...
  - Preference learning: "If user states a preference, update ~/.klausbot/identity/USER.md to record it"
