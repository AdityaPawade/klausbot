...
1. **canStreamToChat(bot, chatId)** - Check if draft streaming is available:
   ```typescript
...
2. **streamToTelegram(bot, chatId, prompt, config, options?)** - Main streaming function:
   - Parameters:
     - bot: Bot instance
     - chatId: Telegram chat ID
     - prompt: User message text
     - config: StreamConfig (enabled, throttleMs)
     - options?: { model?: string, additionalInstructions?: string, messageThreadId?: number }
   - Returns: `Promise<{ result: string, cost_usd: number }>`
   ...
   export async function streamToTelegram(
     ...
     let finalResult = { result: "", cost_usd: 0 };

     const generator = streamClaudeResponse(prompt, {
       model: options?.model,
     ...
     try {
       ...
       // Note: async generator return value needs special handling
       finalResult = { result: accumulated, cost_usd: 0 };

       // Send final draft update
       await bot.api.sendMessageDraft(chatId, draftId, accumulated, {
         message_thread_id: options?.messageThreadId,
       }).catch(() => {});
   ...
   Key points:
   - Draft ID is simple incrementing counter (Telegram overwrites by ID)
   - Throttle updates to config.throttleMs (default 500ms)
   - Always send final draft before returning
   - Catch errors on draft updates to not abort on transient failures

3. Update streamClaudeResponse to:
   - Accept options object with signal, model, additionalInstructions
   - Track cost_usd from final event
   - Return both result and cost_usd
...
- streamToTelegram sends throttled draft updates via sendMessageDraft
- Final draft sent before return
  </done>
</task>
...
2. In processMessage(), after building effectiveText and before the Claude query, add streaming path:
   ...
   if (canStream) {
     ...
     try {
       ...
       log.warn({ err, chatId: msg.chatId }, "Streaming failed, falling back to batch");
     }
   }
...
1. **In streaming.ts - Error handling:**
   - Wrap generator iteration in try/catch
   - On error, still attempt to send final message with accumulated text
   - Log errors but don't crash

   ```typescript
   // In streamToTelegram, wrap the streaming loop:
   try {
     for await (const chunk of generator) {
       // ... existing throttled update logic ...
     }
   } catch (streamErr) {
     log.error({ streamErr, chatId }, "Stream error, sending partial");
     // Send partial result if we have any
...
2. **In streaming.ts - Timeout handling:**
   - Add timeout to streamClaudeResponse (use existing DEFAULT_TIMEOUT from spawner pattern)
   - Kill claude process on timeout, return partial result if available

3. **In gateway.ts - Graceful degradation:**
...
4. **Ensure final message ALWAYS sent:**
