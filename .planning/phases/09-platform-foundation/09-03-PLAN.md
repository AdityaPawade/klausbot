...
- Keep existing exports (config, loadConfig)
  ...
  <verify>
    - `npx tsc --noEmit` passes
    - `npx tsx -e "import { getJsonConfig } from './src/config/index.js'; console.log(getJsonConfig())"` shows defaults
  </verify>
  <done>
...
2. Place after existing commands (pairing, hook, etc.)
  ...
  <verify>
    - `npx tsc --noEmit` passes
    - `npx tsx src/index.ts config validate` shows validation output
    - Without TELEGRAM_BOT_TOKEN: shows error, exits 1
  </verify>
  <done>
