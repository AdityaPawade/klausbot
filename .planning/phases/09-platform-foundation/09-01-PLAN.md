...
must_haves:
  truths:
    ...
    - "Platform info includes displayName, isWSL, arch, nodeVersion"
  artifacts:
...
Use Node.js built-in modules only (node:os, node:fs). No external dependencies.
  ...
  <verify>
    - `npx tsx src/platform/detect.ts` imports without error
...
- PlatformInfo contains all required fields
