...
const SYSTEM =
  "You are klausbot, a Telegram personal assistant. Reply concisely. When the user asks for an action a tool can do, call the appropriate tool with valid arguments. When the user just chats, reply naturally without calling tools.";
...
const TOOLS = [
  {
    ...
    function: {
      name: "create_cron",
      description:
        "Schedule a recurring task with cron syntax (5 fields). Convert natural language times to cron format.",
      parameters: {
        type: "object",
        properties: {
          name: { type: "string" },
  ...
  },
  {
    ...
    function: {
      name: "list_crons",
      description: "List all crons for a chat",
      parameters: {
        type: "object",
        properties: { chat_id: { type: "number" } },
        required: ["chat_id"],
      },
    },
  },
  {
    ...
    function: {
      ...
      description:
        "Search past conversations and memory for what the user previously said.",
      parameters: {
        type: "object",
...
const TASKS = [
  ...
  {
    id: "schedule",
    prompt: "Set a cron at 9am weekdays to drink water. chat_id 1097409126.",
    expectTool: "create_cron",
  },
  {
    id: "list",
    prompt: "List my crons. chat_id 1097409126.",
    expectTool: "list_crons",
  },
  {
    id: "memory",
    prompt: "What do I know about FPGA?",
    expectTool: "search_memories",
  },
  {
    id: "leap",
    prompt:
      "If today is 27 April 2026, what is the next leap year? Reply with just the year number.",
    expectTool: null,
  },
  {
    id: "chat-2",
    prompt: "Tell me one fun fact in one sentence.",
    expectTool: null,
  },
];
...
async function main() {
  ...
  for (let pass = 1; pass <= 2; pass++) {
    console.log(
      `--- Pass ${pass} ${pass === 1 ? "(cold)" : "(warm — same prefix should hit cache)"} ---`,
    );
  ...
  console.log(`\n=== Summary ===`);
  console.log(
    `Pass 1 (cold): ${sum(pass1)}ms total, score ${sumScore(pass1)}/${TASKS.length}, avg ${Math.round(sum(pass1) / TASKS.length)}ms/task`,
  );
  console.log(
    `Pass 2 (warm): ${sum(pass2)}ms total, score ${sumScore(pass2)}/${TASKS.length}, avg ${Math.round(sum(pass2) / TASKS.length)}ms/task`,
  );
  console.log(`Speedup: ${(sum(pass1) / sum(pass2)).toFixed(2)}x`);

  // Cache hit analysis
  const totalCacheBytes = results.reduce((a, b) => a + (b.cached ?? 0), 0);
  const totalPromptBytes = results.reduce(
    (a, b) => a + (b.promptTokens ?? 0),
    0,
  );
  console.log(
    `Cache utilization: ${Math.round((totalCacheBytes / totalPromptBytes) * 100)}% of prompt tokens cached`,
  );
}
